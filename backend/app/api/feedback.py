...
def _build_improvement_prompt(req: FeedbackRequest, logged_at: datetime, client_id: int | str) -> str:
    ...
    lines = [
        ...
        "",
        "Proposed fix:",
        "Before suggesting a fix, answer these diagnostic questions:",
        "  Q1. What data would the correct answer require — which table(s) and columns?",
        "  Q2. Is the data gap in the DB (wrong query), the agent prompt (wrong instruction),",
        "      or the tool logic (wrong filter/sort/join)?",
        "  Q3. Can you reproduce this with a direct SQL query against on24master?",
        "      If so, write that query.",
        "  Q4. What is the minimal change — one line of SQL, one prompt rule, or one tool param?",
        "",
        "Answer:",
        "(LLM to complete)",
        "=" * 60,
