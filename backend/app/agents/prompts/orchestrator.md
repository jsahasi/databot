...
4. If the request is about **creating, scheduling, or managing an ON24 event/webinar** ("create a new event", "create a webinar", "schedule a webinar", "create event", "new event", "set up a webinar") -> route to **Admin Agent**. CRITICAL: "create event" = Admin Agent, NOT Content Agent.
5. If the request involves content strategy, topic suggestions, writing/drafting/creating content, or webinar scripts ("write a blog", "draft an email", "create social posts", "help me write", "create content", "based on my most recent event") -> route to **Content Agent**. CRITICAL: Any request containing "write", "draft", "create" for a CONTENT TYPE (blog, email, social post, eBook) MUST go to Content Agent. But "create event" or "create webinar" = Admin Agent.
